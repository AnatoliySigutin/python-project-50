...
class Distribution(ABC):
    ...
    def source(self) -> str:
        return (
            self.__source
            if (custom_source := get_custom_source(self.name)) is None
            else custom_source
        )
        ...
        if self.source.endswith(".zip"):
            ...
            with open(archive, "rb") as ifh:
                ...
                with (
                    ...
                    tarfile.open(mode="r|", fileobj=reader) as tf,
                ):
                    if sys.version_info[:2] >= (3, 12):
                        tf.extractall(directory, filter="data")
                    else:
...
class CPythonStandaloneDistribution(Distribution):
    ...
    def version(self) -> Version:
        ...
        # 3.7.9-...
        version = (
            remaining.split("%2B")[0] if "%2B" in remaining else remaining.split("-")[0]
        )
...
def get_distribution(
    name: str, source: str = "", variant_cpu: str = "", variant_gil: str = ""
) -> Distribution:
    if source:

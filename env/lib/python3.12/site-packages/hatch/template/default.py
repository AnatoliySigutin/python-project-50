...
class DefaultTemplate(TemplateInterface):
    ...
    def initialize_config(self, config):
        ...
        config["package_metadata_file_path"] = (
            f'src/{config["package_name"]}/__about__.py'
        )
...
"""
        ...
        if not self.plugin_config["src-layout"]:
            config["package_metadata_file_path"] = (
                f'{config["package_metadata_file_path"][4:]}'
            )
        ...
        if license_data:
            if len(license_data) == 1:
                ...
                license_text = get_license_text(
                    config, license_id, text, self.creation_time
                )
                ...
                for license_id, text in license_data.items():
                    ...
                    files.append(
                        File(Path("LICENSES", f"{license_id}.txt"), license_text)
                    )
            ...
            for template_file in files:
                if template_file.path.name.endswith(".py"):
                    template_file.contents = (
                        config["license_header"] + template_file.contents
                    )
...
def get_license_text(config, license_id, license_text, creation_time):
    if license_id == "MIT":
        license_text = license_text.replace(
            "<year>", f"{creation_time.year}-present", 1
        )
        license_text = license_text.replace(
            "<copyright holders>", f'{config["name"]} <{config["email"]}>', 1
        )
        ...
        license_text = license_text.replace(
            "<owner>", f'{config["name"]} <{config["email"]}>', 1
        )

        ...
        if self._env_requires_complex is None:
            ...
            for i, entry in enumerate(requires, 1):
                ...
                try:
                    ...
                    message = (
                        f"Requirement #{i} in `tool.hatch.env.requires` is invalid: {e}"
                    )
    ...
    def env_collectors(self):
        if self._env_collectors is None:
            ...
            for collector, config in collectors.items():
                if not isinstance(config, dict):
                    message = (
                        f"Field `tool.hatch.env.collectors.{collector}` must be a table"
                    )
                    raise TypeError(message)
    ...
    def envs(self):
        ...
        if self._envs is None:
            ...
            for collector, collector_config in self.env_collectors.items():
                collector_class = self.plugin_manager.environment_collector.get(
                    collector
                )
                ...
                for (
                    env_name,
                    data,
                ) in environment_collector.get_initial_config().items():
            ...
            for env_name, raw_initial_config in config.items():
                ...
                if not isinstance(overrides, dict):
                    message = (
                        f"Field `tool.hatch.envs.{env_name}.overrides` must be a table"
                    )
                    raise TypeError(message)
                ...
                for platform, options in platform_overrides.items():
                    ...
                    current_cached_overrides["platform"].append(
                        (platform, current_platform, options)
                    )
                ...
                for env_var, options in env_var_overrides.items():
                    ...
                    current_cached_overrides["env"].append(
                        (env_var, environ[env_var], options)
                    )
                ...
                if not isinstance(matrices, list):
                    message = (
                        f"Field `tool.hatch.envs.{env_name}.matrix` must be an array"
                    )
                    raise TypeError(message)
                ...
                matrix_data = all_matrices[env_name] = {
                ...
                for i, raw_matrix in enumerate(matrices, 1):
                    ...
                    for result in product(*variables.values()):
                        ...
                        for variable, options in matrix_overrides.items():
                            if not isinstance(options, dict):
                                message = f"Field `tool.hatch.envs.{env_name}.overrides.matrix.{variable}` must be a table"
                                raise TypeError(message)
                            ...
                            cached_matrix_overrides.append(
                                (variable, variable_values[variable], options)
                            )

                        # Construct the environment name
                        final_matrix_name_format = new_config.pop(
                            "matrix-name-format", matrix_name_format
                        )
                        env_name_parts = []
                        for j, (variable, value) in enumerate(variable_values.items()):
                            if j == 0 and python_selected:
                                new_config["python"] = value
                                env_name_parts.append(
                                    value if value.startswith("py") else f"py{value}"
                                )
                            else:
                                env_name_parts.append(
                                    final_matrix_name_format.format(
                        ...
                        for pattern, options in name_overrides.items():
                            ...
                            cached_name_overrides.append(
                                (pattern, new_env_name, options)
                            )
                        ...
                        if "py" in variable_values:
                            all_envs[new_env_name] = {
                                "python": variable_values.pop("py"),
                                **variable_values,
                            }
            ...
            for internal_name in get_internal_env_config():
                ...
                # Matrix
                    ...
                    self._internal_matrices[internal_name] = self._matrices.pop(
                        internal_name
                    )
                    for env_name in [
                        env_name
                        for env_name in self._envs
                        if env_name.startswith(f"{internal_name}.")
                    ]:
        ...
        if self._scripts is None:
            ...
            for script_name, commands in config.items():
                commands[:] = expand_script_commands(
                    script_name, commands, config, seen, active
                )
    ...
    def finalize_env_overrides(self, option_types):
        ...
        for environments in (self.envs, self.internal_envs):
            for env_name, config in environments.items():
                for override_name, data in self._cached_env_overrides.get(
                    env_name, {}
                ).items():
        ...
        if possible_script in config:
            expanded_commands.extend(
                format_script_commands(
                    commands=expand_script_commands(
                        possible_script, config[possible_script], config, seen, active
                    ),
                    args=args,
...
def _populate_default_env_values(env_name, data, config, seen, active):
    ...
    for key, value in template_config.items():
        ...
        if key == "scripts":
            scripts = (
                data["scripts"] if "scripts" in data else data.setdefault("scripts", {})
            )

...
def apply_overrides(
    env_name, source, condition, condition_value, options, new_config, option_types=None
):
    ...
    for raw_option, data in options.items():
        ...
        if override_type in TYPE_OVERRIDES:
            ...
            if _resolve_condition(
                env_name, option, source, condition, condition_value, data
            ):
...
def _apply_override_to_mapping(
    ...
    if isinstance(data, str):
        ...
        for i, entry in enumerate(data, 1):
            if isinstance(entry, str):
                ...
                if not isinstance(value, str):
                    ...
                    raise TypeError(message)
                if _resolve_condition(
                    env_name, option, source, condition, condition_value, entry, i
                ):
                    new_mapping[key] = value
            else:
                message = (
        ...
        message = f"Field `tool.hatch.envs.{env_name}.overrides.{source}.{condition}.{option}` must be a string or an array"
        raise TypeError(message)
...
def _resolve_condition(
    ...
    location = (
        "field" if condition_index is None else f"entry #{condition_index} in field"
    )
    ...
    if "env" in condition_config:
        ...
        for env_var, value in required_env_vars.items():
            if env_var not in environ or (
                value is not None and value != environ[env_var]
            ):
                return False

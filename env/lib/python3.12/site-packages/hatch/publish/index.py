...
class IndexPublisher(PublisherInterface):
    ...
    def get_repos(self):
        ...
        for repo, data in defined_repos.items():
            if isinstance(data, str):
                ...
                self.app.abort(
                    f"Hatch config field `publish.index.repos.{repo}` must be a string or a mapping"
                )
            elif "url" not in data:
                self.app.abort(
                    f"Hatch config field `publish.index.repos.{repo}` must define a `url` key"
                )
            else:
                repos[repo] = data
    ...
    def publish(self, artifacts: list, options: dict):
        ...
        repo = (
            options["repo"]
            if "repo" in options
            else self.plugin_config.get("repo", "main")
        )
        ...
        for artifact in recurse_artifacts(artifacts, self.root):
            ...
            for field in ("name", "version"):
                if field not in data:
                    self.app.abort(
                        f"Missing required field `{field}` in artifact: {artifact}"
                    )
            ...
            try:
                ...
                self.app.abort(
                    f"Error uploading to repository: {index.repo} - {e}".replace(
                        index.auth, "*****"
                    )
                )
            else:
        ...
        for project_name, versions in project_versions.items():
            ...
            for version in versions:
                self.app.display_info(
                    str(index.urls.project.child(project_name, version, "").to_iri())
                )

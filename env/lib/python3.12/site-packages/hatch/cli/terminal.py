...
class BorrowedStatus(TerminalStatus):
    ...
    def __active(self) -> bool:
        return (
            self.__status is not None and self.__status._live.is_started
        )  # noqa: SLF001
...
class Terminal:
    def __init__(
        self, *, verbosity: int, enable_color: bool | None, interactive: bool | None
    ):
        ...
        for option, style in styles.items():
            ...
            if default_level:
                try:
                    ...
                    errors.append(
                        f"Invalid style definition for `{option}`, defaulting to `{default_level}`: {e}"
                    )
    ...
    def display_warning(
        self, text="", *, stderr=True, indent=None, link=None, **kwargs
    ):
        if self.verbosity < -1:
            return
    ...
    def display_debug(
        self, text="", level=1, *, stderr=True, indent=None, link=None, **kwargs
    ):
    ...
    def display_table(
        ...
        *,
        ...
        table = Table(
            title=title, show_lines=show_lines, title_style="", **table_options
        )
        ...
        for column_title, indices in list(columns.items()):
            if indices:
                table.add_column(
                    column_title, style="bold", **column_options.get(column_title, {})
                )
            else:
    ...
    def _output(
        self, text="", style=None, *, stderr=False, indent=None, link=None, **kwargs
    ):
        if indent:

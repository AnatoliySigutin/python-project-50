...
@click.option(
    ...
    "-p",
    is_flag=True,
    help="Whether or not to exclude environment dependencies",
)
@click.option(
    "--env-only",
    "-e",
    is_flag=True,
    help="Whether or not to exclude project dependencies",
)
@click.pass_obj
...
@click.option(
    ...
    "show_lines",
    is_flag=True,
    help="Whether or not to show lines between table rows",
)
@click.option(
    "--ascii",
    "force_ascii",
    is_flag=True,
    help="Whether or not to only use ASCII characters",
)
@click.pass_obj
def table(app, project_only, env_only, show_lines, force_ascii):
    ...
    from hatch.utils.dep import (
        get_normalized_dependencies,
        get_project_dependencies_complex,
        normalize_marker_quoting,
    )
    ...
    for all_requirements, table_title in (
        ...
        normalized_requirements = [
            Requirement(d) for d in get_normalized_dependencies(all_requirements)
        ]
        ...
        for i, requirement in enumerate(normalized_requirements):
            ...
            if requirement.marker:
                columns["Markers"][i] = normalize_marker_quoting(
...
@click.option(
    ...
    "all_features",
    is_flag=True,
    help="Whether or not to include the dependencies of all features",
)
@click.pass_obj
def requirements(app, project_only, env_only, features, all_features):
    ...
    dependencies_complex, optional_dependencies_complex = (
        get_project_dependencies_complex(environment)
    )
    ...
    if features:
        for raw_feature in features:
            ...
            if feature not in optional_dependencies_complex:
                app.abort(
                    f"Feature `{feature}` is not defined in field `project.optional-dependencies`"
                )

...
@click.option(
    "--user",
    "-u",
    envvar=PublishEnvVars.USER,
    help="The user with which to authenticate [env var: `HATCH_INDEX_USER`]",
)
@click.option(
    "--auth",
    "-a",
...
@click.option(
    ...
    "-n",
    is_flag=True,
    help="Disable prompts, such as for missing required fields",
)
@click.option(
    "--initialize-auth",
    is_flag=True,
    help="Save first-time authentication information even if nothing was published",
)
@click.option(
    "--publisher",
    "-p",
...
@click.option(
    ...
    "-y",
    is_flag=True,
    help="Confirm without prompting when the plugin is disabled",
)
@click.pass_obj
def publish(
    app,
    ...
    if publisher.disable and not (
        yes or (not no_prompt and app.confirm(f"Confirm `{publisher_name}` publishing"))
    ):

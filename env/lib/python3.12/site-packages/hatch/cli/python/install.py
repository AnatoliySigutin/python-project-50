...
@click.option(
    "--private", is_flag=True, help="Do not add distributions to the user PATH"
)
...
@click.option(
    ...
    "-d",
    "directory",
    help="The directory in which to install distributions, overriding configuration",
)
@click.pass_obj
def install(
    app: Application,
    *,
    ...
    for name in compatible:
        ...
        if name in installed:
            ...
            if not needs_update:
                app.display_warning(
                    f"The latest version is already installed: {installed_dist.version}"
                )
                continue
        ...
        app.display_success(
            f'{"Updated" if needs_update else "Installed"} {name} @ {dist.path}'
        )

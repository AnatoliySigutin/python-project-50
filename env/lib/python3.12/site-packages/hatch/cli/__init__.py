...
def hatch(
    ...
    app = Application(
        ctx.exit, verbosity=verbose - quiet, enable_color=color, interactive=interactive
    )
    ...
    if config_file:
        ...
        if not app.config_file.path.is_file():
            app.abort(
                f"The selected config file `{app.config_file.path}` does not exist."
            )
    ...
    if app.config.mode == "project":
        ...
        if possible_project is None:
            app.display_warning(
                f"Unable to locate project {app.config.project}, defaulting to the current directory"
            )
        else:

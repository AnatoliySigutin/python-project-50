...
@click.command(
    short_help="Format and lint source code",
    context_settings={"ignore_unknown_options": True},
)
@click.argument("args", nargs=-1)
@click.option(
    "--check", is_flag=True, help="Only check for errors rather than fixing them"
)
...
@click.option(
    "--sync",
    is_flag=True,
    help="Sync the default config file with the current version of Hatch",
)
@click.pass_obj
def fmt(
    app: Application,
    *,
    ...
    for context in app.runner_context(["hatch-static-analysis"]):
        ...
        if legacy_config_path := app.project.config.config.get("format", {}).get(
            "config-path", ""
        ):
            app.display_warning(

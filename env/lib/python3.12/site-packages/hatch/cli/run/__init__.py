    ...
    if first_arg.endswith(".py") and (script := Path(first_arg)).is_file():
        ...
        if metadata:
            ...
            if (
                ...
                and (requires_python := metadata.get("requires-python")) is not None
            ):
                import re
                import sys
                ...
                distributions = [
                    name for name in DISTRIBUTIONS if re.match(r"^\d+\.\d+$", name)
                ]
                ...
                for distribution in distributions:
                    ...
                    app.abort(
                        f"Unable to satisfy Python version constraint: {requires_python}"
                    )

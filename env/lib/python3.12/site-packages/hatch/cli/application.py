...
class Application(Terminal):
    ...
    def get_environment(self, env_name: str | None = None) -> EnvironmentInterface:
        ...
        if self.project.location.is_file():
            data_directory = isolated_data_directory = (
                self.data_dir / "env" / environment_type / ".scripts"
            )
        elif is_isolated_environment(env_name, config):
            data_directory = isolated_data_directory = (
                self.data_dir / "env" / ".internal" / env_name
            )
        else:
        ...
        with context.env.command_context():
            try:
                resolved_commands = list(
                    context.env.resolve_commands(context.shell_commands)
                )
            except Exception as e:  # noqa: BLE001
                self.abort(str(e))

            first_error_code = None
            should_display_command = not context.hide_commands and (
                self.verbose or len(resolved_commands) > 1
            )
    ...
    def ensure_environment_plugin_dependencies(self) -> None:
        self.ensure_plugin_dependencies(
            self.project.config.env_requires_complex,
            wait_message="Syncing environment plugin requirements",
        )

    def ensure_plugin_dependencies(
        self, dependencies: list[Requirement], *, wait_message: str
    ) -> None:
        if not dependencies:
            return
        ...
        if app_path := os.environ.get("PYAPP"):
            ...
            executable = self.platform.check_command_output(
                [app_path, management_command, "python-path"]
            ).strip()
    ...
    def env_metadata(self) -> EnvironmentMetadata:
        return EnvironmentMetadata(
            self.data_dir / "env" / ".metadata", self.project.location
        )
...
class EnvironmentMetadata:
    ...
    def update_dependency_hash(
        self, environment: EnvironmentInterface, dependency_hash: str
    ) -> None:
    ...
    def _write(
        self, environment: EnvironmentInterface, metadata: dict[str, Any]
    ) -> None:
        import json
    ...
    def _metadata_file(self, environment: EnvironmentInterface) -> Path:
        ...
        return (
            self._storage_dir / environment.config["type"] / f"{environment.name}.json"
        )

...
def version(app: Application, desired_version: str | None):
    ...
    with app.project.location.as_cwd():
        if not (
            "version" in app.project.metadata.dynamic
            or app.project.metadata.hatch.metadata.hook_config
            ...
            with (
                ...
                environment.build_environment(app.project.metadata.build.requires),
            ):
                status.stop()

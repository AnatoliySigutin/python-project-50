...
@click.option(
    "--json", "as_json", is_flag=True, help="Whether or not to output in JSON format"
)
    ...
    if as_json:
        ...
        for environments in (app.project.config.envs, app.project.config.internal_envs):
            for env_name, config in environments.items():
                ...
                extra_dependencies = environment.environment_dependencies[
                    num_dependencies:
                ]
                ...
                if environment.scripts:
                    new_config["scripts"] = {
                        script: list(environment.resolve_commands([script]))
    ...
    for i, (matrix_name, matrix_data) in enumerate(target_matrices.items()):
        ...
        if dependencies:
            ...
            matrix_columns["Dependencies"][i] = "\n".join(
                sorted(normalized_dependencies)
            )
        ...
        if config.get("scripts"):
            matrix_columns["Scripts"][i] = "\n".join(
                sorted(
                    ...
                    if app.verbose or not script.startswith("_")
                )
            )
    ...
    for i, (env_name, config) in enumerate(standalone_envs):
        ...
        if environment.environment_dependencies_complex:
            standalone_columns["Dependencies"][i] = "\n".join(
                sorted(
                    {
                        get_normalized_dependency(d)
                        for d in environment.environment_dependencies_complex
                    }
                )
            )

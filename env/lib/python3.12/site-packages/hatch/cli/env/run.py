...
@click.option(
    "--filter", "-f", "filter_json", help="The JSON data used to select environments"
)
@click.option(
    "--force-continue",
    is_flag=True,
    help="Run every command and if there were any errors exit with the first code",
)
@click.option(
    "--ignore-compat",
    is_flag=True,
    help="Ignore incompatibility when selecting specific environments",
)
@click.pass_obj
def run(
    app: Application,
    *,
    ...
    for env_name in ordered_env_names:
        if env_name in project.config.matrices:
            ...
            environments.extend(
                select_environments(env_data, included_variables, excluded_variables)
            )
        else:
    ...
    if not environments:
        ...
        app.abort(
            f'Variable selection is unsupported for non-matrix environments: {", ".join(ordered_env_names)}'
        )

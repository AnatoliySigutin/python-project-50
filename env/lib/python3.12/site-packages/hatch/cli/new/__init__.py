...
@click.option(
    ...
    "interactive",
    is_flag=True,
    help="Interactively choose details about the project",
)
@click.option(
    "--cli",
    "feature_cli",
    is_flag=True,
    help="Give the project a command line interface",
)
@click.option(
    "--init", "initialize", is_flag=True, help="Initialize an existing project"
)
...
def new(app, name, location, interactive, feature_cli, initialize, setuptools_options):
    ...
    templates = []
    for template_name, template_class in sorted(
        template_classes.items(), key=lambda item: -item[1].PRIORITY
    ):
        if template_name in plugin_config:
            templates.append(
                template_class(
                    plugin_config.pop(template_name),
                    app.cache_dir,
    ...
    if not templates:
        app.abort(
            f'None of the defined plugins were found: {", ".join(sorted(plugin_config))}'
        )
    elif plugin_config:
        app.abort(
            f'Some of the defined plugins were not found: {", ".join(sorted(plugin_config))}'
        )
    ...
    for template in templates:
        for possible_template_file in template.get_files(
            config=deepcopy(template_config)
        ):
            template_file = (
                possible_template_file(
                    deepcopy(template_config), template.plugin_config
                )
                if possible_template_file.__class__ is not File
                else possible_template_file
            )
            if template_file.path is None or (
                initialize and str(template_file.path) != "pyproject.toml"
            ):
                continue
        ...
        for path in paths:
            if path.is_dir():
                recurse_directory(
                    path,
                    tree.add(
                        f"[bold magenta][link={app.platform.format_file_uri(path)}]{escape(path.name)}"
                    ),
                )
            else:
                tree.add(
                    f"[green][link={app.platform.format_file_uri(path)}]{escape(path.name)}"
                )

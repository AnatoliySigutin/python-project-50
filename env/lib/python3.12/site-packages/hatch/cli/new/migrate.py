...
def _parse_setup_cfg(kwargs):
    ...
    if setup_cfg.has_section("options"):
        ...
        if "install_requires" in options and "install_requires" not in kwargs:
            kwargs["install_requires"] = _parse_dependencies(
    ...
    if (
        setup_cfg.has_section("options.extras_require")
        and "extras_require" not in kwargs
    ):
...
def setup(**kwargs):
    ...
    package_name = package_path = package_source = (
        packages[0].split(".")[0].lower().strip()
    )
    ...
    for collaborator in ("author", "maintainer"):
        ...
        if f"{collaborator}_email" in kwargs:
            collaborator_emails.extend(
                collaborator_email.strip()
                for collaborator_email in kwargs[f"{collaborator}_email"].split(",")
            )

        for collaborator_name, collaborator_email in itertools.zip_longest(
            collaborator_names, collaborator_emails
        ):
            data = {}
    ...
    if "classifiers" in kwargs:
        ...
        for i, index in enumerate(fixed_indices):
            project_metadata["classifiers"].insert(
                final_index, project_metadata["classifiers"].pop(index - i)
            )
    ...
    if kwargs.get("data_files", []):
        ...
        for shared_directory, relative_paths in kwargs["data_files"]:
            ...
            for relative_directory, files in sorted(relative_files.items()):
                if not os.path.isdir(relative_directory) or set(
                    os.listdir(relative_directory)
                ) != set(files):
    ...
    output = _collapse_data(
        output, {"requires": project_data["build-system"]["requires"]}
    )
    ...
    if os.path.isfile(project_file):
        ...
        for section in ("build-system", "project"):
            for pattern in (rf"^\[{section}].*?(?=^\[)", rf"^\[{section}].*"):
                current_contents = re.sub(
                    pattern, "", current_contents, flags=re.MULTILINE | re.DOTALL
                )
...
def migrate(root, setuptools_options, sys_paths):
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        shutil.copytree(
            root,
            repo_dir,
            ignore=shutil.ignore_patterns(".git", ".tox"),

...
@click.command(
    short_help="Run tests", context_settings={"ignore_unknown_options": True}
)
@click.argument("args", nargs=-1)
@click.option(
    "--randomize", "-r", is_flag=True, help="Randomize the order of test execution"
)
...
@click.option(
    "--cover-quiet",
    is_flag=True,
    help="Disable coverage reporting after tests, implicitly enabling --cover",
)
@click.option(
    "--all", "-a", "test_all", is_flag=True, help="Test all environments in the matrix"
)
@click.option(
    "--python", "-py", help="The Python versions to test, equivalent to: -i py=..."
)
@click.option(
    "--include",
    "-i",
...
@click.option(
    ...
    "-s",
    is_flag=True,
    help="Show information about environments in the matrix",
)
@click.pass_context
def test(
    ctx: click.Context,
    *,
    ...
    if show:
        ...
        ctx.invoke(
            ...
            envs=(
                ctx.obj.project.config.internal_matrices["hatch-test"]["envs"]
                if app.verbose
                else ["hatch-test"]
            ),
        )
    ...
    if retries is None and retry_delay is not None:
        app.abort(
            "The --retry-delay option requires the --retries option to be set as well."
        )
    ...
    if test_all:
        ...
        selected_envs.extend(
            select_environments(test_envs, included_variables, excluded_variables)
        )
    else:
        ...
        candidate_names.sort(
            key=lambda name: test_envs[name].get("python") != current_version
        )
    ...
    patched_coverage = PatchedCoverageConfig(
        app.project.location, app.data_dir / ".config"
    )
    ...
    for context in app.runner_context(
        selected_envs, ignore_compat=multiple_possible, display_header=multiple_possible
    ):
        ...
        if (
            seconds_delay := context.env.config.get("retry-delay", retry_delay)
        ) is not None:

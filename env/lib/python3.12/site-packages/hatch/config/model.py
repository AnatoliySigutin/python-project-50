...
class LazilyParsedConfig:
    ...
    def raise_error(self, message, *, extra_steps=()):
        ...
        raise ConfigurationError(
            message, location=" -> ".join([*self.steps, field, *extra_steps])
        )
            ...
            if "projects" in self.raw_data:
                ...
                for name, data in projects.items():
                    if isinstance(data, str):
                        project_data[name] = ProjectConfig(
                            {"location": data}, ("projects", name)
                        )
                    elif isinstance(data, dict):
                        project_data[name] = ProjectConfig(data, ("projects", name))
                    else:
                        self.raise_error(
                            "must be a string or table", extra_steps=(name,)
                        )
    ...
    def publish(self):
        if self._field_publish is FIELD_TO_PARSE:
            if "publish" in self.raw_data:
                ...
                self._field_publish = self.raw_data["publish"] = {
                    "index": {"repo": "main"}
                }
    ...
    def data(self):
        if self._field_data is FIELD_TO_PARSE:
            if "data" in self.raw_data:
                ...
                self._field_data = self.raw_data["data"] = user_data_dir(
                    "hatch", appauthor=False
                )
    ...
    def cache(self):
        if self._field_cache is FIELD_TO_PARSE:
            if "cache" in self.raw_data:
                ...
                self._field_cache = self.raw_data["cache"] = user_cache_dir(
                    "hatch", appauthor=False
                )
    ...
    def licenses(self):
        if self._field_licenses is FIELD_TO_PARSE:
            if "licenses" in self.raw_data:
                ...
                self._field_licenses = LicensesConfig(
                    licenses, (*self.steps, "licenses")
                )
            else:
                licenses = {}

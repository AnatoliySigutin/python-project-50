...
class VirtualEnvironment(EnvironmentInterface):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            ...
            app_virtual_env_path = (
                self.isolated_data_directory / project_name / project_id / venv_name
            )

        # Explicit path
        chosen_directory = self.get_env_var_option("path") or self.config.get(
            "path", ""
        )
        if chosen_directory:
            ...
            self.virtual_env_path = (
                ...
                if isabs(chosen_directory)
                else (self.root / chosen_directory).resolve()
            )
        elif project_is_script:
            self.storage_path = self.virtual_env_path = (
                self.isolated_data_directory / venv_name
            )
        ...
        self.virtual_env = self.virtual_env_cls(
            self.virtual_env_path, self.platform, self.verbosity
        )
    ...
    def uv_path(self) -> str:
        ...
        if not (
            ...
            or is_default_environment(
                env_name, self.app.project.config.internal_envs[env_name]
            )
        ):
...
"""
    ...
    def remove(self):
        ...
        if (
            self.storage_path != self.platform.home / ".virtualenvs"
            and self.storage_path.is_dir()
        ):
            entries = [entry.name for entry in self.storage_path.iterdir()]
            if not entries or (
                entries == [".gitignore"] and self.root in self.storage_path.parents
            ):
        ...
        with self.safe_activation():
            self.platform.check_command(
                self.construct_pip_install_command(
                    [self.apply_features(str(self.root))]
                )
            )
    ...
    def install_project_dev_mode(self):
        with self.safe_activation():
            self.platform.check_command(
                self.construct_pip_install_command(
                    ["--editable", self.apply_features(str(self.root))]
                )
            )
    ...
    def build_environment(self, dependencies):
        ...
        with self.get_env_vars(), self.build_virtual_env:
            if not dependencies_in_sync(
                ...
                self.platform.check_command(
                    self.construct_pip_install_command(dependencies)
                )
    ...
    def get_interpreter_resolver_env(self) -> dict[str, str]:
        ...
        env["PATH"] = (
            internal_path
            if old_path is None
            else f"{old_path}{os.pathsep}{internal_path}"
        )
    ...
    def _interpreter_is_compatible(self, interpreter: PythonInfo) -> bool:
        return (
            ...
            and (
                self.skip_install
                or self._python_constraint.contains(interpreter.version_str)
            )
        )
    ...
    def _get_concrete_interpreter_path(self, python_version: str = "") -> str | None:
        ...
        if python_version:
            ...
            for resolver in resolvers:
                if (
                    concrete_path := resolver(self._preferred_python_version)
                ) is not None:
                    return concrete_path
    ...
    def _resolve_external_interpreter_path(self, python_version: str) -> str | None:
        if (
            existing_path := self._find_existing_interpreter(python_version)
        ) is not None:
    ...
    def _resolve_internal_interpreter_path(self, python_version: str) -> str | None:
        if (
            available_distribution := self._get_available_distribution(python_version)
        ) is not None:
            with self.app.status(
                f"Installing Python distribution: {available_distribution}"
            ):
    ...
    def _get_available_distribution(self, python_version: str = "") -> str | None:
        ...
        if not python_version:
            ...
            available_distributions = [
                d for d in compatible_distributions if not d.startswith("pypy")
            ]
    ...
    def _python_constraint(self) -> SpecifierSet:
        ...
        return SpecifierSet(
            self.metadata.config.get("project", {}).get("requires-python", "")
        )

...
class EnvironmentInterface(ABC):
    ...
    def env_vars(self) -> dict:
        ...
        if self._env_vars is None:
            ...
            if not isinstance(env_vars, dict):
                message = (
                    f"Field `tool.hatch.envs.{self.name}.env-vars` must be a mapping"
                )
                raise TypeError(message)

            for key, value in env_vars.items():
                if not isinstance(value, str):
                    message = f"Environment variable `{key}` of field `tool.hatch.envs.{self.name}.env-vars` must be a string"
                    raise TypeError(message)
    ...
    def env_include(self) -> list[str]:
        ...
        if self._env_include is None:
            ...
            if not isinstance(env_include, list):
                message = (
                    f"Field `tool.hatch.envs.{self.name}.env-include` must be an array"
                )
                raise TypeError(message)
    ...
    def env_exclude(self) -> list[str]:
        ...
        if self._env_exclude is None:
            ...
            if not isinstance(env_exclude, list):
                message = (
                    f"Field `tool.hatch.envs.{self.name}.env-exclude` must be an array"
                )
                raise TypeError(message)
            ...
            with self.apply_context():
                for option in ("dependencies", "extra-dependencies"):
                    ...
                    for i, entry in enumerate(dependencies, 1):
                        ...
                        try:
                            dependencies_complex.append(
                                Requirement(self.metadata.context.format(entry))
                            )
    ...
    def environment_dependencies(self) -> list[str]:
        ...
        if self._environment_dependencies is None:
            self._environment_dependencies = [
    ...
    def dependencies_complex(self):
        if self._dependencies_complex is None:
            ...
            if (not self.skip_install and self.dev_mode) or self.features:
                ...
                dependencies_complex, optional_dependencies_complex = (
                    get_project_dependencies_complex(self)
                )
                ...
                for feature in self.features:
                    ...
                    all_dependencies_complex.extend(
                        optional_dependencies_complex[feature].values()
                    )
    ...
    def dependencies(self) -> list[str]:
        ...
        if self._dependencies is None:
            self._dependencies = [
                str(dependency) for dependency in self.dependencies_complex
            ]
    ...
    def platforms(self) -> list[str]:
        ...
        if self._platforms is None:
            ...
            if not isinstance(platforms, list):
                message = (
                    f"Field `tool.hatch.envs.{self.name}.platforms` must be an array"
                )
                raise TypeError(message)
    ...
    def skip_install(self) -> bool:
        ...
        if self._skip_install is None:
            skip_install = self.config.get(
                "skip-install", not self.metadata.has_project_file()
            )
    ...
    def dev_mode(self) -> bool:
        ...
        if self._dev_mode is None:
            ...
            if not isinstance(dev_mode, bool):
                message = (
                    f"Field `tool.hatch.envs.{self.name}.dev-mode` must be a boolean"
                )
                raise TypeError(message)
        ...
        if self._features is None:
            ...
            for i, feature in enumerate(features, 1):
                ...
                if (
                    ...
                    not in self.metadata.core.optional_dependencies
                ):
                    message = (
    ...
    def description(self) -> str:
        ...
        if self._description is None:
            ...
            if not isinstance(description, str):
                message = (
                    f"Field `tool.hatch.envs.{self.name}.description` must be a string"
                )
                raise TypeError(message)
        ...
        if self._scripts is None:
            ...
            for field in ("extra-scripts", "scripts"):
                ...
                if not isinstance(script_config, dict):
                    message = (
                        f"Field `tool.hatch.envs.{self.name}.{field}` must be a table"
                    )
                    raise TypeError(message)
            ...
            for script_name, commands in config.items():
                commands[:] = expand_script_commands(
                    self.name, script_name, commands, config, seen, active
                )
    ...
    def app_status_dependency_state_check(self):
        ...
        if not self.skip_install and (
            "dependencies" in self.metadata.dynamic
            or "optional-dependencies" in self.metadata.dynamic
        ):

...
def get_normalized_dependencies(requirements: list[Requirement]) -> list[str]:
    normalized_dependencies = {
        get_normalized_dependency(requirement) for requirement in requirements
    }
    ...
    data = "".join(
        sorted(
            ...
            for normalized_dependency in {
                get_normalized_dependency(req) for req in requirements
            }
        )
    ).encode("utf-8")
...
def get_project_dependencies_complex(
    ...
    if not environment.metadata.hatch.metadata.hook_config or dependencies_in_sync(
        ...
        optional_dependencies_complex.update(
        ...
        try:
            ...
            environment.app.abort(
                f"Environment `{environment.name}` is incompatible: {e}"
            )
        ...
        with (
            environment.root.as_cwd(),
            environment.build_environment(environment.metadata.build.requires),
        ):
            command = [
                "python",
                "-u",
            ...
            for feature, optional_dependencies in project_metadata.get(

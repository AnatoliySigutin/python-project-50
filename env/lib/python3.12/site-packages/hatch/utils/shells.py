...
class ShellManager:
    ...
    def enter_cmd(
        self, path: str, args: Iterable[str], exe_dir: Path
    ) -> None:  # noqa: ARG002
        self.environment.platform.exit_with_command(
            [path or "cmd", "/k", str(exe_dir / "activate.bat")]
        )

    def enter_powershell(
        self, path: str, args: Iterable[str], exe_dir: Path
    ) -> None:  # noqa: ARG002
        self.environment.platform.exit_with_command(
            [
    ...
    def enter_xonsh(self, path: str, args: Iterable[str], exe_dir: Path) -> None:
        if self.environment.platform.windows:
            ...
            self.spawn_linux_shell(
                ...
                callback=lambda terminal: terminal.sendline(
                    f"$PATH.insert(0, {str(exe_dir)!r})"
                ),
            )
    ...
    def enter_nu(
        ...
        self.environment.platform.exit_with_command(
            [executable, "-e", f"overlay use {str(activation_script)!r}"]
        )

    def enter_tcsh(self, path: str, args: Iterable[str], exe_dir: Path) -> None:
        self.spawn_linux_shell(
            path or "tcsh", args or ["-i"], script=exe_dir / "activate.csh"
        )

    def enter_csh(self, path: str, args: Iterable[str], exe_dir: Path) -> None:
        self.spawn_linux_shell(
            path or "csh", args or ["-i"], script=exe_dir / "activate.csh"
        )
        ...
        def spawn_linux_shell(
            ...
            *,
            ...
            terminal = pexpect.spawn(
                path, args=list(args or ()), dimensions=(lines, columns)
            )

            def sigwinch_passthrough(
                sig: int, data: FrameType | None
            ) -> None:  # noqa: ARG001

...
from hatchling.builders.constants import (
...
class BuilderConfig:
    ...
    def include_spec(self) -> pathspec.GitIgnoreSpec | None:
        ...
        all_include_patterns.extend(
            f"/{relative_path.replace(os.sep, '/')}/" for relative_path in self.packages
        )
    ...
    def skip_excluded_dirs(self) -> bool:
        if "skip-excluded-dirs" in self.target_config:
            ...
            if not isinstance(skip_excluded_dirs, bool):
                message = (
                    "Field `tool.hatch.build.skip-excluded-dirs` must be a boolean"
                )
                raise TypeError(message)
    ...
    def dev_mode_dirs(self) -> list[str]:
        ...
        if "dev-mode-dirs" in self.target_config:
            ...
            dev_mode_dirs_location = (
                f"tool.hatch.build.targets.{self.plugin_name}.dev-mode-dirs"
            )
        else:
    ...
    def versions(self) -> list[str]:
        ...
        for i, version in enumerate(versions, 1):
            if not isinstance(version, str):
                message = f"Version #{i} in field `tool.hatch.build.targets.{self.plugin_name}.versions` must be a string"
                raise TypeError(message)
        ...
        if isinstance(raw_sources, list):
            for i, source in enumerate(raw_sources, 1):
                if not isinstance(source, str):
                    message = (
                        f"Source #{i} in field `{sources_location}` must be a string"
                    )
                    raise TypeError(message)
            ...
            for source, path in raw_sources.items():
                ...
                sources[normalize_relative_directory(source) if source else source] = (
                    normalized_path
                )
        else:
            message = (
    ...
    def force_include(self) -> dict[str, str]:
        if "force-include" in self.target_config:
            ...
            force_include_location = (
                f"tool.hatch.build.targets.{self.plugin_name}.force-include"
            )
        else:
    ...
    def only_include(self) -> dict[str, str]:
        if "only-include" in self.target_config:
            ...
            only_include_location = (
                f"tool.hatch.build.targets.{self.plugin_name}.only-include"
            )
        else:

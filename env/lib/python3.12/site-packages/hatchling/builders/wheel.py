...
class WheelArchive:
    ...
    def add_file(self, included_file: IncludedFile) -> tuple[str, str, str]:
        ...
        with (
            open(included_file.path, "rb") as in_file,
    ...
    def write_shared_script(
        self, included_file: IncludedFile, contents: str | bytes
    ) -> tuple[str, str, str]:
        relative_path = f"{self.shared_data_directory}/scripts/{normalize_archive_path(included_file.distribution_path)}"
        ...
        return self.write_file(
            ...
            contents,
            mode=(
                normalize_file_permissions(file_stat.st_mode)
                if self.reproducible
                else file_stat.st_mode
            ),
        )
...
class WheelBuilder(BuilderInterface):
    ...
    def add_shared_data(
        ...
        shared_data.update(
            normalize_inclusion_map(build_data["shared_data"], self.root)
        )
    ...
    def add_shared_scripts(
        ...
        shared_scripts.update(
            normalize_inclusion_map(build_data["shared_scripts"], self.root)
        )
...
"""
        ...
        for major_version in known_major_versions:
            for minor_version in range(max_version_part):
                ...
                if self.metadata.core.python_constraint.contains(
                    f"{major_version}.{minor_version}.{max_version_part}"
                ):
    ...
    def get_best_matching_tag(self) -> str:
        ...
        if sys.platform == "darwin":
            ...
            tag_parts[2] = process_macos_plat_tag(
                tag_parts[2], compat=self.config.macos_max_compat
            )

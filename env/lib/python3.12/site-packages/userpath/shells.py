...
class Xonsh(Shell):
    def config(self, location, front=True):
        ...
        if front:
            contents = "\n".join(
                "$PATH.insert(0, {!r})".format(location)
                for location in reversed(locations)
            )
        else:
            contents = "\n".join(
                "$PATH.append({!r})".format(location) for location in locations
            )
...
class Zsh(Shell):
    def config(self, location, front=True):
        ...
        return {
            path.join(zdotdir, ".zshrc"): contents,
            path.join(zdotdir, ".zprofile"): contents,
        }

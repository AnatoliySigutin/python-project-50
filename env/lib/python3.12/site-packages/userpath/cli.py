...
@userpath.command(
    context_settings=CONTEXT_SETTINGS, short_help="Prepends to the user PATH"
)
...
@click.option(
    ...
    help=(
        ...
        "effect on Windows. The default shells are: {}".format(
            ", ".join(sorted(DEFAULT_SHELLS))
        )
    ),
)
@click.option(
...
@click.option(
    "-f", "--force", is_flag=True, help="Update PATH even if it appears to be correct."
)
@click.option(
    "-q", "--quiet", is_flag=True, help="Suppress output for successful invocations."
)
def prepend(locations, shells, all_shells, home, force, quiet):
    ...
    if not force:
        for location in locations:
            if up.in_current_path(location):
                ...
                location, shells=shells, all_shells=all_shells, home=home
            ):
                echo_warning(
                    (
    ...
    try:
        up.prepend(
            locations, shells=shells, all_shells=all_shells, home=home, check=True
        )
...
@userpath.command(
    context_settings=CONTEXT_SETTINGS, short_help="Appends to the user PATH"
)
@click.argument("locations", required=True, nargs=-1)
@click.option(
    "-s",
...
def verify(locations, shells, all_shells, home, quiet):
    ...
    for location in locations:
        if up.in_current_path(location):
            ...
            echo_warning(
                "The directory `{}` is in PATH, pending a shell restart!".format(
                    location
                )
            )
            sys.exit(2)
